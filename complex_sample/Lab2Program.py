...
class Lab2Program:
    ...
    def initialize_vrep_client(self):
        ...
        while count < num_tries:
            ...
            self.clientID=vrep.simxStart('127.0.0.1',8000,True,True,5000,5) #Timeout=5000ms, Threadcycle=5ms
            if self.clientID!=-1:
    ...
    def global_map_process(self, im):
        ...
        if self.blurred_paths is None:
            print "Computed"
            blurred_map = skimage.filters.gaussian(walls, sigma=2)
    ...
    def robot_code(self):
        ...
        while (time.time() - t) < 200:
            ...
            print finalVector
            #rey rey
            if finalVector[0]!=0 and finalVector[1]!=0:
               finalUnitVector = finalVector / np.linalg.norm(finalVector)
            else:
              finalUnitVector=finalVector
